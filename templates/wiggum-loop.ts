 ...
 *
 * This is a DOMAIN-AGNOSTIC template. Customize prompts.ts and hooks.ts
 * for your specific use case. Run with --sanity-check to verify the loop
 * works before customizing.
 *
 * Core behavior:
 ...
 * - CAN read files, run tests, edit project status docs.
 * - CANNOT delete files via Bash (rm/rmdir/unlink, git clean).
 * - CANNOT run mutating git commands (only status/diff/log/show).
 *
...
import { buildBuilderPrompt, buildVerifierPrompt } from "./prompts.js";
...
// ------------------------------------
// Load .env from repo root (for ANTHROPIC_API_KEY etc.)
// ------------------------------------
...
// Project plan directory — should contain a CLAUDE.md with project status
const PLAN_DIR = path.resolve(
  REPO_ROOT,
  process.env.WIGGUM_PLAN_DIR ?? "harness/project",
);
...
const MAX_LOOPS = SANITY_CHECK
  ? 1
  : Number(process.env.WIGGUM_MAX_LOOPS ?? "20");
    ...
    if (!fs.existsSync(planFile)) {
      throw new Error(
        `Project plan not found at ${planFile}.\n` +
          `Create a CLAUDE.md in your project plan directory or set WIGGUM_PLAN_DIR.`,
      );
    }
  }
...
async function main(): Promise<void> {
  ...
  if (SANITY_CHECK) {
    ...
    console.log("  WIGGUM LOOP — Autonomous Builder-Verifier");
  }
  console.log("============================================================");
  if (!SANITY_CHECK) {
  ...
  for (let i = 1; i <= MAX_LOOPS; i++) {
    ...
    const vPrompt = SANITY_CHECK
      ...
      : buildVerifierPrompt(REPO_ROOT, PLAN_DIR);
